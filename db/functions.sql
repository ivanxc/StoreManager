CREATE OR REPLACE FUNCTION add_products(IN titles varchar(255)[],
					...
					IN barcodes integer[]) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION add_admission(IN title varchar(255)) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION add_admission_product(IN product_id integer,
						 ...
						 IN amount integer) RETURNS void AS $$
BEGIN
   INSERT INTO admission_product (product_id, admission_id, price, amount) values (product_id, admission_id, price, amount);
END;
$$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION get_all_products() RETURNS TABLE (id integer, title varchar(255), amount integer, price double precision, barcode integer) AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION update_product_price(IN id integer, IN new_price integer) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION update_product_amount(IN id integer, IN additional_amount integer) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION change_product_amount(IN id integer, IN new_amount integer) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION add_cashier(IN name varchar(255)) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION add_selling(IN product_id integer, IN check_id integer, IN amount integer, IN price double precision) RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION add_check(IN price double precision, IN cashier_id integer) RETURNS void AS $$
BEGIN
